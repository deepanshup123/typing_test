...
Typed_Text.addEventListener('keydown', (event) => {
    let right_type = 0;
    let wrong_type = 0;
    let final_array = [];
    ...
    stores = stores.replace(/\s+/g, ' ');

    let store_the_assign = verify.split(' ');
    let store_the_typed = stores.split(" ");

    // Handle space key separately
    if (event.key === ' ') {
        store_the_typed.push('');
    }
